...
static uint8_t blufi_wifi_retry = 0;
...
static void initialize_sntp(void)
{
    ...
    BLUFI_INFO("List of configured NTP servers:");

    for (uint8_t i = 0; i < SNTP_MAX_SERVERS; ++i){
        if (sntp_getservername(i)){
            BLUFI_INFO("server %d: %s", i, sntp_getservername(i));
        } else {
            // we have either IPv4 or IPv6 address, let's print it
            char buff[INET6_ADDRSTRLEN];
            ip_addr_t const *ip = sntp_getserver(i);
            if (ipaddr_ntoa_r(ip, buff, INET6_ADDRSTRLEN) != NULL)
                BLUFI_INFO("server %d: %s", i, buff);
        }
    }
}
...
static void obtain_time(void)
{
    ...
    BLUFI_INFO("The current date/time in Jerusalem is: %s", strftime_buf);
...
static void blufi_record_wifi_conn_info(int rssi, uint8_t reason)
{
...
static void blufi_wifi_connect(void)
{
    blufi_wifi_retry = 0;
...
{
    bool ret;
    if (gl_sta_is_connecting && blufi_wifi_retry++ < WIFI_CONNECTION_MAXIMUM_RETRY) {
...
{
    char* msg = malloc(data_len+1);
    memset(msg, 0, data_len+1);
    memcpy(msg, data, data_len);

    cJSON* root = cJSON_Parse(msg);
    cJSON* opcodeStr = cJSON_GetObjectItem(root, "opcode");
    opcodes opcode = -1;
    if (opcodeStr)
    {
        opcode = opcodeStr->valueint;
        BLUFI_INFO("Received opcode %d", opcode);
    }

    switch(opcode)
    {
        case MQTT_CONFIG:
            handle_mqtt_config(root);
        break;

        default:
            BLUFI_ERROR("Received unknown opcode %d, ignoring", opcode);
        break;
    }
...
static void blufi_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
    ...
    case ESP_BLUFI_EVENT_RECV_CUSTOM_DATA:
        ...
        handle_custom_message(param->custom_data.data, param->custom_data.data_len);
        break;
...
/* Initialize the NimBLE host configuration. */
    ...
    rc = ble_svc_gap_device_name_set("Blabla");
    assert(rc == 0);
